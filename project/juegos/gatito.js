...
function operar() {
    ...
    const respuesta = prompt('Responde la siguiente operación aritmética: ' + operacion);
}
...
function ponerFicha(event){
    ...
    if (!partidaAcabada && botonPulsado.innerHTML == "") {
        const operacion = generarOperacionAritmetica();
    const respuesta = prompt('Responde la siguiente operación aritmética: ' + operacion);
         if (evaluarRespuesta(operacion, respuesta)) {
...
function evaluarRespuesta(operacion, respuesta) {
  try {
    const resultado = eval(operacion);
    return parseFloat(respuesta) === resultado;
  } catch (error) {
    return false;
  }
}
